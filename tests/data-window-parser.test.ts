...
import {
  ...
  dataHexToDecimal,
  formatDataValue
} from '../src/renderer/src/data-window-parser'
test('data-window-parser regression coverage', () => {
  ...
  for (const invalid of [
    ...
    assert.throws(() => compileDataPattern(invalid), Error, `Invalid pattern: ${invalid}`)
  ...
  console.log(
    'Data window parser: split frames, multiple frames, custom fields, invalid templates, isolation and bounded buffering passed.'
  )
  ...
  console.log(
    'DEC conversion: big-endian example, zero, unsigned 32-bit and precise 64-bit passed.'
  )
