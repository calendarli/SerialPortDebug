...
static SRWLOCK gPairLock = SRWLOCK_INIT;
static PQUEUE_CONTEXT gEndpoints[SERIALFLOW_MAX_ENDPOINTS] = {0};

static BOOL
ArePeers(_In_ PQUEUE_CONTEXT First, _In_ PQUEUE_CONTEXT Second)
{
    PDEVICE_CONTEXT a = First->DeviceContext;
    PDEVICE_CONTEXT b = Second->DeviceContext;
    if (a->PairId[0] == L'\0' || b->PairId[0] == L'\0') return FALSE;
    return _wcsicmp(a->PairId, b->PairId) == 0 && a->PairSide != b->PairSide;
}

static NTSTATUS
PairRegister(_In_ PQUEUE_CONTEXT QueueContext)
{
    ULONG index;
    AcquireSRWLockExclusive(&gPairLock);
    for (index = 0; index < SERIALFLOW_MAX_ENDPOINTS; index++) {
        if (gEndpoints[index] == NULL) {
            gEndpoints[index] = QueueContext;
            QueueContext->DeviceContext->QueueContext = QueueContext;
...
VOID
QueueUnregister(_In_ PDEVICE_CONTEXT DeviceContext)
{
    ULONG index;
    AcquireSRWLockExclusive(&gPairLock);
    for (index = 0; index < SERIALFLOW_MAX_ENDPOINTS; index++) {
        if (gEndpoints[index] == DeviceContext->QueueContext) {
            gEndpoints[index] = NULL;
            break;
        }
    }
    DeviceContext->QueueContext = NULL;
    ReleaseSRWLockExclusive(&gPairLock);
}
    ...
    *Target = NULL;
    ...
    for (index = 0; index < SERIALFLOW_MAX_ENDPOINTS; index++) {
        if (gEndpoints[index] == Source) {
            ULONG peerIndex;
            if (Source->DeviceContext->PairId[0] == L'\0') {
                peerIndex = index ^ 1;
            ...
            for (peerIndex = 0; peerIndex < SERIALFLOW_MAX_ENDPOINTS; peerIndex++) {
    ...
    )
{
    ...
    return PairRegister(queueContext);
}
