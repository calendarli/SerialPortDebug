...
--*/
{
    ...
    DECLARE_CONST_UNICODE_STRING(pairIdName,        L"PairId");
    DECLARE_CONST_UNICODE_STRING(pairSideName,      L"PairSide");
    DECLARE_UNICODE_STRING_SIZE (comPort,           10);
    DECLARE_UNICODE_STRING_SIZE (pairId,            PAIR_ID_LENGTH);
    ...
    DeviceContext->PairId[0] = L'\0';
    DeviceContext->PairSide = MAXULONG;
    if (NT_SUCCESS(WdfRegistryQueryUnicodeString(key, &pairIdName, NULL, &pairId))) {
        wcscpy_s(DeviceContext->PairId, PAIR_ID_LENGTH, pairId.Buffer);
    }
    WdfRegistryQueryULong(key, &pairSideName, &DeviceContext->PairSide);
